...
def trainLBFGS(train_file, model_file):
    ...
    #     "classias-train -tb -a lbfgs.logistic -pc1=0 -pc2=1 -m %s %s > NUL" %
    #     (model_file, train_file), shell=True)
    retcode = subprocess.Popen(['cmd',
                                "/C C:\\classias-1.1_win32\\classias-train -tb -a lbfgs.logistic "
                                "-pc1=0 -pc2=1 -m %s %s > NUL" % (model_file, train_file)], shell=True)
    return retcode
...
def testLBFGS(test_file, model_file):
    ...
    """
    output = "work/output"
    # Bollegala version LINUX
    # retcode = subprocess.call("cat %s | classias-tag -m %s -t > %s" %
    #                           (test_file, model_file, output), shell=True)

    # Windows)
    retcode = subprocess.call("type %s | C:\\classias-1.1_win32\\classias-tag -m %s -t > %s" %
                              (test_file, model_file, output), shell=True)
    F = open(output)
    accuracy = 0
    correct = 0
...
def generateFeatureVectors(domain):
    ...
    for (mode, label) in [("train", "positive"), ("train", "negative"), ("train", "unlabeled"),
                          ("test", "positive"), ("test", "negative")]:
        fname = "%s/%s-%s-%s.pk" % (domain, domain, mode, label)
        fvects = FeatGen.process_file(fname, label)
        writeFeatureVectorsToFile(fvects, "work/%s/%s.%s" % (domain, mode, label))
    pass
...
def generateAll():
    ...
    """
    domains = ["books", "electronics", "cds", "clothes"]
...
def evaluate_SA(source, target, project):
    ...
    trainFileName = "work\\%s-%s\\trainVects.SpectralAlignment" % (source, target)
    testFileName = "work\\%s-%s\\testVects.SpectralAlignment" % (source, target)
    ...
    modelFileName = "work\\%s-%s\\model.SpectralAlignment" % (source, target)
